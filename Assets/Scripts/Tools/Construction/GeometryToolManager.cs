...
public class GeometryToolManager
{
    ...
    public static void Place(World world, Player player, MainCamera mainCamera)
    {
        ...
        {
            //+ Floor
            if (mainCamera.raycast.normal.y >= 0.75f && Geometry.Dictionary[GameFlow.selectedGeometry].placedOn == Geometry.PlacedOn.FLOOR)
            {
                ...
                Geometry.Dictionary[GameFlow.selectedGeometry].Place(GameFlow.selectedGeometry, mainCamera.raycast.point, 
                    new Vector3(0, yRotation, 0), Vector3.one, true);
            }
            //+ Wall
            else if (mainCamera.raycast.normal.y == 0 && Geometry.Dictionary[GameFlow.selectedGeometry].placedOn == Geometry.PlacedOn.WALL)
            {
                int yRotation;
                ...
                {
                    yRotation = 0;
                    Geometry.Dictionary[GameFlow.selectedGeometry].Place(GameFlow.selectedGeometry,
